...
\begin{itemize}
    ...
    We will use a bounded verification tool.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|c|c|l|X|c|}
        ...
        REQ\_000        & C                 & Reset                 & Correct init and graceful recover from reset mid-transaction                              & F/H/U         \\ \midrule
        REQ\_001        & C                 & Input accept          & Producer \texttt{in\_valid} is high and DUT \texttt{in\_ready} is high accepts input      & F             \\ \midrule
        REQ\_002        & C                 & Input stall           & DUT \texttt{in\_ready} is low and producer \texttt{in\_valid} is high                     & F             \\ \midrule
        REQ\_003        & C                 & Output accept         & Consumer \texttt{out\_ready} is high and DUT \texttt{out\_valid} is high accepts output   & F             \\ \midrule
        REQ\_004        & C                 & Output stall          & Consumer \texttt{out\_ready} is low and DUT \texttt{out\_valid} is high                   & F/H           \\ \midrule
        REQ\_005        & C                 & Output stability      & \texttt{gcd\_out} is stable while \texttt{out\_valid} is high                             & F/H           \\ \bottomrule \toprule
        ...
        REQ\_008        & D                 & Zero operands         & Correct \texttt{gcd\_out} for $a=0$ and/or $b=0$, early exit                              & F/H/U         \\ \midrule
        REQ\_009        & D                 & Equal operands        & Correct \texttt{gcd\_out} for $a=b$, early exit                                           & F/H/U         \\ \bottomrule \toprule
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|c|p{3.9cm}|X|p{1.7cm}|}
        ...
        F               & \texttt{idle\_done}, \texttt{idle\_run}, \texttt{run\_done}, \texttt{done\_idle}  & Prove FSM accurately transitions between states based on handshakes and data path evaluations                             & REQ\_001, REQ\_003, REQ\_008, REQ\_009, REQ\_012  \\ \midrule
...
In practice the objectives are a complete (100\%) coverage of the functional description and possibly a complete (100\%) code coverage.
...
\section{Diagrams}
This section will introduce the FSM in form of a diagram.
Every nameless arrow implies that all inputs that have not been specified translate in that state.
The plan for the UVM architecture is displayed in the Implementation chapter \ref{cha:implementation}
\begin{figure}[htbp]
  \centering
  \includesvg[width=0.5\textwidth, inkscapelatex=false]{FSM.svg}
  \caption{Finite State machine of the DUT}
\end{figure}